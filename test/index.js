...
function handleAddChoreTimeRequest(intent, session, response) {
    ...
    if (choreOut.error) {
        // invalid city. move to the dialog
        repromptText = "Try another Chore.";
        // if we received a value for the incorrect city, repeat it to the user, otherwise we received an empty slot
        speechOutput = "I'm sorry, I don't have any data for " + choreOut.chore + ". " + repromptText : repromptText;
    ...
    if (!date) {
        // Invalid date. set city in session and prompt for date
        session.attributes.chore = choreOut;
        repromptText = "Please try again saying something like today or yesterday";
        speechOutput = "I'm sorry, I didn't understand that date. " + repromptText;
    ...
    // all slots filled, either from the user or by default values. Move to final request
    getAddChoreResponse(choreOut, date, response);
}
...
function handleTellChoreTimeRequest(intent, session, response) {
    ...
    // all slots filled, either from the user or by default values. Move to final request
    getTellChoreResponse(choreOut, response);
}
...
/**
 * this responds to when the user wants to add achore
 */
function getAddChoreResponse(choreOut, date, response) {
    ...
    // Issue the request, and respond to the user
        ...
        if (err) {
            speechOutput = "Sorry, there's a wrong thing. Try again soon.";
        } else {
            speechOutput = date.displayDate + " you " + choreOut.chore ". I remembered that.";
        }

        response.tellWithCard(speechOutput, "Chore Tracker", speechOutput)
 ...
 * this responds to when the user wants to add achore
 */
function getTellChoreResponse(choreOut, response) {
    ...
    // Issue the request, and respond to the user
        ...
        if (err) {
            ...
            speechOutput = "I am telling you that you " + choreOut.chore ". I remembered that.";
        }
